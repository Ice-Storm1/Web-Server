...
function renderTickets() {
  ...
  container.innerHTML = filteredTickets.map(ticket => {
    const unreadCount = !admin ? getUnreadAdminMessages(ticket.id) : 0;
    return `
    <div class="ticket-card" onclick="${admin ? `openTicketEdit('${ticket.id}')` : `openChat('${ticket.id}')`}" style="${!admin ? 'cursor: pointer;' : ''}">
      <div class="ticket-card-header">
        ...
        ${!admin && unreadCount > 0 ? `<span class="admin-msg-badge" title="Nuovi messaggi dall'Admin">💬 ${unreadCount}</span>` : ''}
      </div>
...
function getUnreadAdminMessages(ticketId) {
  const chats = JSON.parse(localStorage.getItem(CHAT_KEY) || '{}');
  const chatKey = `ticket_${ticketId}`;
  const messages = chats[chatKey] || [];
  const user = getCurrentUser();
  const lastRead = parseInt(localStorage.getItem(`lastRead_${ticketId}_${user.username}`) || '0');
  return messages.filter(m => m.isAdmin === true && m.timestamp > lastRead).length;
}
...
function openChat(ticketId) {
  ...
  if (messages.length > 0) {
    ...
    localStorage.setItem(`lastRead_${ticketId}_${user.username}`, new Date(lastMsg.timestamp).getTime().toString());
  }
